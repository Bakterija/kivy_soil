...
<MDMenuItem>
    ...
    canvas.before:
        Color:
            rgba: app.mtheme.col_bblue_transp06 if self.selected else [0, 0, 0, 0]
        Rectangle:
            size: self.size
            pos: self.pos
        Color:
            rgba: app.mtheme.col_bblue_transp06 if self.hovering else [0, 0, 0, 0]
        Rectangle:
            size: self.size
            pos: self.pos
    Label:
...
class MDDropdownMenu(BoxLayout):
    button_height = NumericProperty(int(cm(0.8)))
    ...
    def display_menu(self, caller):
        ...
        target_width = int(self.width_mult * self.button_height)
        # If we're wider than the Window...
        if target_width > Window.width:
            # ...reduce our multiplier to max allowed.
            target_width = int(
                Window.width / self.button_height) * self.button_height

        target_height = self.button_height * len(self.items)
